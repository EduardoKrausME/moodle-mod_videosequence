    ...
    const sendProgress = async(config, state, previous, current, duration, elapsed) => {
        ...
        try {
            const result = await Ajax.call([{
                methodname: 'mod_videosequence_update_progress',
                args: {
                    cmid: config.cmid,
                    current,
                    duration,
            ...
            config.lastposition =
                Number(result.lastposition) || config.lastposition;
    ...
    const initHtml5 = (config, video) => {
        ...
        video.addEventListener('loadedmetadata', () => {
            if (
                ...
                Number(config.lastposition) < video.duration - 2
            ) {
        ...
        document.addEventListener(
            ...
            event => {
                if (
                    config.allowseek ||
                    Number(event.detail.time) <=
                    Number(config.lastposition) + 2
                ) {
                    video.currentTime =
                        Number(event.detail.time);
